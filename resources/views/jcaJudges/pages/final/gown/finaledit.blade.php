<form action="{{ route('finalScoreUpdate') }}" method="POST" id="editSwimwear">
    @csrf
    <div class="p-0">
        <div class="card">
                    ...
                    <tbody>
                        @foreach ($contestants as $contestant)
                            <tr>
                                ...
                                <td>
                                    <input type="number" class="form-control w-75 finalBeauty" placeholder="Beauty ...." value="{{$contestant->beauty}}"
                                            min="1" max="50" name="beauty[{{ $contestant->id }}]">
                                </td>
                                <td>
                                    <input type="number" class="form-control w-75 finalPoise" value="{{$contestant->poise}}"
                                            placeholder="Poise grace ...." min="1" max="50"
                                            name="finalPoise[{{ $contestant->id }}]">
                                </td>
                                <td>
                                    <input type="number" class="form-control w-75 finalProjection" value="{{$contestant->projection}}"
